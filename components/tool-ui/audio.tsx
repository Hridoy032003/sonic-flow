    ...
    <div className="w-full">
      ...
      <div className="flex items-center justify-between gap-8 h-12">
        ...
        <div className="flex items-center gap-3 w-1/4 min-w-[200px]">
          ...
          <button className="text-slate-300 hover:text-rose-500 transition-colors ml-2"><Heart size={16} /></button>
        </div>

        {/* Playback Controls */}
        <div className="flex-1 flex flex-col items-center gap-1.5">
           <div className="flex items-center gap-6">
              <button className="text-slate-400 hover:text-slate-600 transition-colors"><Shuffle size={14} /></button>
              <button className="text-slate-600 hover:text-slate-900 transition-colors"><SkipBack size={18} fill="currentColor" /></button>
              <button 
                onClick={togglePlay}
                className="w-9 h-9 flex items-center justify-center bg-slate-900 text-white rounded-full hover:scale-105 active:scale-95 transition-all"
              >
                {isPlaying ? <Pause size={18} fill="currentColor" /> : <Play size={18} fill="currentColor" className="ml-0.5" />}
              </button>
              <button className="text-slate-600 hover:text-slate-900 transition-colors"><SkipForward size={18} fill="currentColor" /></button>
              <button className="text-slate-400 hover:text-slate-600 transition-colors"><Repeat size={14} /></button>
           </div>
           
           <div className="w-full max-w-lg flex items-center gap-3">
              <span className="text-[10px] font-bold text-slate-400 w-10 text-right">{formatTime(currentTime)}</span>
              <div className="flex-1 h-1 bg-slate-100 rounded-full relative overflow-hidden group">
                 <div 
                   className="absolute left-0 inset-y-0 bg-slate-400 group-hover:bg-indigo-500 transition-colors"
